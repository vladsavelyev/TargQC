...
from Utils.reporting.reporting import ReportSection, Metric, MetricStorage, SampleReport, PerRegionSampleReport, \
    get_col_widths, write_tsv_rows, write_txt_rows, Row
...
def make_targqc_reports(work_dir, sample, target_bed, gene_keys_list):
    ...
    per_gene_report = make_per_gene_report(work_dir, sample, target_bed, tc.features_bed_fpath,
        tc.features_no_genes_bed, gene_by_name_and_chrom, avg_depth=avg_depth)
...
def make_per_gene_report(work_dir, sample, target_bed, features_bed, features_no_genes_bed,
                         gene_by_name_and_chrom, avg_depth=None):
    info('-' * 70)
    info('Detailed exon-level report')

    if tc.reuse_intermediate and verify_file(sample.targetcov_detailed_tsv, silent=True):
        ...
        if features_bed or target_bed:
            per_gene_report = _generate_report_from_bam(work_dir, sample, target_bed, features_no_genes_bed,
...
def _generate_report_from_bam(work_dir, sample, target_bed, features_no_genes_bed, gene_by_name_and_chrom, avg_depth):
    depth_thresholds = tc.depth_thresholds
    if avg_depth:
    ...
    for (bed, feature) in zip([target_bed, features_no_genes_bed], ['amplicons', 'exons']):  # features are canonical
        ...
        sambamba_depth_output_fpath = sambamba_depth(work_dir, bed, sample.bam, depth_thresholds, reuse=tc.reuse_intermediate)
